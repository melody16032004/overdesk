...
// --- TREE VIEW COMPONENT (Giữ nguyên) ---
const JsonNode = ({
  name,
  value,
  isLast,
...
export const JsonModule = () => {
  ...
  // --- HOTKEYS (NEW) ---
  // Sử dụng useCallback để hàm luôn có state mới nhất (đặc biệt là savedFiles và json)
  const handleSaveHotkey = useCallback(() => {
    // Logic saveToLibrary được đưa vào đây để tái sử dụng
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      // Chỉ bắt sự kiện khi giữ Ctrl hoặc Meta (Mac)
      if (e.ctrlKey || e.metaKey) {
        switch (e.key.toLowerCase()) {
          case "s": // Ctrl + S: Save to Library
            e.preventDefault();
            handleSaveHotkey();
            break;
          case "d": // Ctrl + D: Clear
  ...
  }, [handleSaveHotkey]); // Dependency quan trọng

  // --- ACTIONS ---
  const saveToLibrary = handleSaveHotkey; // Reuse logic
