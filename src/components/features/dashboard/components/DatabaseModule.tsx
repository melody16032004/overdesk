...
// --- ADVANCED SQL ENGINE (Giữ nguyên từ bản trước) ---
...
const compareValues = (rowVal: any, operator: string, queryVal: any) => {
...
const checkWhereClause = (row: any, whereClause?: string) => {
...
const executeQuery = (query: string, tables: DBTable[]): QueryResult => {
  try {
...
const generateValueForColumn = (col: Column, rowId: number) => {
...
export const DatabaseModule = ({
  ...
  const [isLibraryOpen, setIsLibraryOpen] = useState(false); // NEW: Library State
  ...
  const applyTemplate = (templateQuery: string) => {
    ...
    setSqlQuery(query);
    if (window.innerWidth < 768) setIsLibraryOpen(false); // Close drawer on mobile
  };
