...
// --- HELPER: Lấy thông tin OS & Browser ---
...
export const SystemInfoModule = () => {
  ...
  const [loadingIp, setLoadingIp] = useState(false); // Trạng thái đang tải IP
  ...
  const getSignalStrength = (rtt: number) => {
    if (!rtt) return 4;
    if (rtt < 50) return 4;
    if (rtt < 100) return 3;
    if (rtt < 200) return 2;
  ...
  // --- FUNCTION: LẤY IP THẬT (Ưu tiên API chính xác hơn) ---
  const fetchIp = async () => {
    setLoadingIp(true);
    setIpInfo(null); // Reset để hiện loading
    try {
      // Ưu tiên 1: ipapi.co (Thường chính xác vị trí ở VN hơn)
