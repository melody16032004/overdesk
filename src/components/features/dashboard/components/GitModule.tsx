...
// --- TYPES ---
type Tab = "generator" | "github" | "cheatsheet";
...
export const GitModule = () => {
  ...
  // 👇 History State (Lưu object {login, avatar})
  ...
  useEffect(() => {
    ...
    switch (genAction) {
      case "smart_commit":
        // Logic tạo Conventional Commits
        const scope = commitScope ? `(${commitScope})` : "";
        cmd = `git add .\ngit commit -m "${commitType}${scope}: ${
          genInput1 || "description"
        }"`;
        break;
      case "config":
      ...
      case "push":
        ...
        break;
      case "force_push": // Dangerous command
        cmd = `git push origin ${genInput1 || "main"} --force`;
        break;
      case "tag":
        cmd = `git tag -a v${genInput1 || "1.0.0"} -m "${
          genInput2 || "Release version 1.0"
  ...
  const fetchGitHub = async (targetUser = username) => {
    ...
    setLoading(true);
    setUsername(targetUser); // Sync input
    try {
      ...
      addToHistory(dataProfile); // Save to history
