...
export const TimerModule = () => {
  ...
  const { mode, timeLeft, initialTime, isActive, sessions, lastUpdated } =
    timerState;
  ...
  const handleTimerComplete = () => {
    ...
    if (Notification.permission === "granted") {
      new Notification("Time's up!", {
        body: `${MODE_CONFIG[mode].label} finished.`,
      });
    }
  ...
  return (
    <div className="h-full flex flex-col items-center relative overflow-hidden bg-slate-50/50 dark:bg-white/5">
      ...
      <div className="w-full p-2 flex justify-center gap-1 z-10">
        {(Object.keys(MODE_CONFIG) as Array<keyof typeof MODE_CONFIG>).map(
          (m) => {
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative w-full">
        ...
        {!isActive && (
          <div className="absolute right-4 top-1/2 -translate-y-1/2 flex flex-col gap-1 animate-in fade-in slide-in-from-right-2">
            <button
              ...
              className="p-1 rounded-lg hover:bg-slate-200 dark:hover:bg-white/10 text-slate-400 hover:text-indigo-500 transition-colors"
            >
              <ChevronUp size={16} />
            </button>
            <button
        ...
        <div className="relative">
          ...
          <div
            className={`absolute inset-0 flex flex-col items-center justify-center transition-colors duration-300 ${currentConfig.color}`}
          >
            {formatTime(timeLeft)}
            <div className="flex items-center gap-1 mt-1">
              <span className="flex gap-1">
                {[...Array(4)].map((_, i) => (
                  <div
                    key={i}
                    className={`w-1.5 h-1.5 rounded-full ${i < sessions % 4 ? "bg-current" : "bg-slate-200 dark:bg-white/10"}`}
                  />
                ))}
              </span>
            </div>
