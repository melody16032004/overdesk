...
import { computeDiff, formatJson } from "./helper/json_tool_helper";
...
export const JsonToolsModule = () => {
  ...
  // --- 2. FORMATTER HANDLERS ---

  const handleFormat = () => {
    setErrorMsg(null);
    try {
      const formatted = formatJson(inputJson); // Giả sử hàm này format JSON string
  ...
  const handleDiff = () => {
    ...
    const res = computeDiff(diffLeft, diffRight); // Giả sử hàm này trả về mảng DiffLine
    setDiffResult(res);
  };

  // --- 4. UTILITY HANDLERS (Copy, Reset) ---
